...
function PatientAppointments() {
  ...
  async function loadPatient() {
    try {
      ...
      setPatient(res.data);
      if (res.data.patientId) loadAppointments(res.data.patientId);
    } catch {
  ...
  useEffect(() => { loadDoctors(); loadPatient(); }, []);
  useEffect(() => {
    if (doctorId) {
...
const availableSchedule = futureSchedules.find(
  ...
  async function bookAppointment() {
    ...
    try {
      ...
      setDoctorId(""); setAppointmentDate(""); setAppointmentTime("");
      setSchedules([]); setSelectedSpecialization("");
  ...
  const selectClass = "w-full bg-white border border-slate-200 text-slate-800 rounded-lg px-4 py-3 text-sm focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500 transition-colors duration-200 shadow-sm disabled:bg-slate-100 disabled:text-slate-400 disabled:cursor-not-allowed";
  const editSelectClass = "w-full bg-white border border-amber-200 text-slate-800 rounded-lg px-4 py-3 text-sm focus:outline-none focus:border-amber-400 focus:ring-1 focus:ring-amber-400 transition-colors duration-200 shadow-sm disabled:bg-slate-100 disabled:text-slate-400 disabled:cursor-not-allowed";
    ...
    <div className="min-h-screen bg-slate-300 px-4 py-12">
      ...
      <div className="relative max-w-5xl mx-auto">
        ...
        <div className={`rounded-2xl p-8 shadow-xl mb-6 border transition-colors duration-300 ${
          isEditing ? "bg-amber-50 border-amber-200" : "bg-white border-slate-200"
        }`}>
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-2">
              <div className={`w-7 h-7 rounded-lg border flex items-center justify-center ${
                isEditing ? "bg-amber-100 border-amber-300" : "bg-indigo-50 border-indigo-200"
              }`}>
                <svg className={`w-3.5 h-3.5 ${isEditing ? "text-amber-600" : "text-indigo-500"}`}
          ...
          <div className="grid gap-4">
            <div>
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  setDoctorId(""); setAppointmentDate(""); setAppointmentTime(""); setSchedules([]);
                }}
            ...
            <div>
              ...
              >
                ...
                {filteredDoctors.map((d) => <option key={d.doctorId} value={d.doctorId}>{d.doctorName}</option>)}
              </select>
            </div>
    ...
    {s.availableDate}
            ...
            <div className={`border-t pt-4 ${isEditing ? "border-amber-200" : "border-slate-100"}`}>
              <button
                ...
                className={`font-semibold px-6 py-3 rounded-lg text-sm tracking-wide transition-colors shadow-sm text-white ${
                  isEditing
          ...
          <div className="flex items-center justify-between px-8 py-5 border-b border-slate-100">
            ...
            <div className="hidden sm:flex items-center gap-2">
              {Object.entries(statusStyles).map(([status, style]) => (
                <span key={status} className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${style}`}>
                  {status}
                </span>
              ))}
            </div>
                  ...
                  <tr>
                    <td colSpan={8} className="px-6 py-12 text-center">
                      <div className="flex flex-col items-center gap-2">
                        ...
                        <p className="text-xs text-slate-300">Book your first appointment above.</p>
                      </div>
                    </td>
                  </tr>
                ) : (
                    ...
                    return (
                      <tr key={a.appointmentId}
                        className={`transition-colors duration-150 ${
                          isRowEditing ? "bg-amber-50 border-l-2 border-l-amber-400" : "hover:bg-slate-50"
                        }`}>
                        <td className="px-5 py-4 text-slate-400 font-mono text-xs">{a.appointmentId}</td>
                        <td className="px-5 py-4 text-slate-800 font-medium whitespace-nowrap">{doc?.doctorName || "—"}</td>
                        <td className="px-5 py-4 text-slate-600 whitespace-nowrap">{doc?.specialization || "—"}</td>
                        ...
                        <td className="px-5 py-4 whitespace-nowrap">
                          <div className="flex items-center gap-2">
                            ...
                            <button
                              ...
                              className="bg-red-50 hover:bg-red-100 text-red-500 border border-red-200 px-3 py-1.5 rounded-lg text-xs font-medium transition-colors whitespace-nowrap"
                            >
                              Cancel
                            </button>
                          </div>
